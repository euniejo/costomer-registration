...
namespace CustomerManagerApp_Ejo_8820817.DataAccess
{
    public class CustomerDbContext : DbContext
    {
        ...
        // Define a property to access the Customers table in the DB:
        ...
        protected override void OnModelCreating(ModelBuilder modelBuilder)
        {
            ...
            modelBuilder.Entity<Invoice>().HasKey(c => c.InvoiceId); // Corrected this line to use InvoiceId as the primary key

            modelBuilder.Entity<PaymentTerms>().HasKey(c => c.PaymentTermsId); // Corrected this line to use InvoiceId as the primary key

            modelBuilder.Entity<InvoiceLineItem>().HasKey(c => c.InvoiceLineItemId); // Corrected this line to use InvoiceId as the primary key

            modelBuilder.Entity<Invoice>().HasOne(c => c.Customer).WithMany(m => m.Invoices).HasForeignKey(c => c.CustomerId);

            modelBuilder.Entity<Invoice>().HasOne(c => c.PaymentTerm).WithMany(m => m.Invoices).HasForeignKey(c => c.PaymentTermsId);

            modelBuilder.Entity<InvoiceLineItem>().HasOne(c => c.Invoice).WithMany(m => m.InvoiceLineItems).HasForeignKey(c => c.InvoiceId);
            ...
            // use this as a place to seed the DB with some data, e.g. 8 customers
            ...
            // use this as a place to seed the DB with some data, e.g. 12 Invoices
            ...
            // use this as a place to seed the DB with some data, e.g. 22 InvoiceLineItem
